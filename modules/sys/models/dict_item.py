...
# 数据库模型
class DictItemOrmModel(BaseOrmModel):
    """
    字典项数据库模型
    """
    ...
    # SQLAlchemy字段定义
    dictId: str = db_types.Column(db_types.BigInteger, name="dict_id", nullable=False, comment="字典ID")
    name: str = db_types.Column(db_types.String(64), name="name", nullable=False, comment="字典项名称")
...
# Pydantic模型
class DictItem(APIModel):
    """
    字典项Pydantic模型，用于API接口
    """
    # Pydantic字段定义
    dictId: str = Field(None, description="字典ID")
    name: str = Field(None, description="字典项名称")
